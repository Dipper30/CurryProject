...
/**
 * 根据输入值返回unix时间戳，若为字符串'yyyy-MM-dd'格式，返回解析后的时间戳，若字符串格式不正确或不为合法日期，返回-1
 * 若输入值为数字，则判断长度是否为10，若不为10，则返回-1
 * 若无输入值，则返回当前时间unix时间戳
 * @param dateStr 
 * @returns unix时间戳
 */
export const getUnixTSByDateString = (dateStr: string | number): number => {
  if (!dateStr) return getUnixTS()
  if (typeof dateStr == 'number') return dateStr.toString().length == 10 ? dateStr : -1
